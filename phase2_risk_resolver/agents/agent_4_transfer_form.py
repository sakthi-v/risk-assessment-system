"""
Agent 4 - Risk Transfer Form Generator
Generates final transfer form from questionnaire responses
"""
...
def search_knowledge_base(query: str) -> str:
    """Search organizational knowledge base for risk transfer form templates"""
    return search_knowledge_base_function(query)


def generate_transfer_form(api_key: str, risk_context: Dict[str, Any], questionnaire_responses: Dict[str, Any], questionnaire_structure: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    Generate final risk transfer form from questionnaire responses
    ...
    try:
        ...
        agent = Agent(
            role="Risk Transfer Form Generator",
            goal="Generate filled transfer form by mapping questionnaire answers to form structure",
...
"""
        ...
        task = Task(
            description=context,
            agent=agent,
            expected_output="Complete risk transfer form in JSON format"
        )
        
        crew = Crew(agents=[agent], tasks=[task], verbose=False)
        
        print("\n" + "="*80)
        print("🤖 AGENT 4: GENERATING TRANSFER FORM (SAME AS ACCEPTANCE)")
        print("="*80)
